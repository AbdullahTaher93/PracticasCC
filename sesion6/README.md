...
Los documentos permiten documentos incrustados/embebidos:
...
```
sudo docker run --name mongotest -d mongo:latest
```

Entramos a la shell de nuestro contenedor:
```
sudo docker exec -it mongotest bash 
```
...
Ahora estás usando la base de datos ``azureuser`` .
...
```
db.MyFirstCollection.find({"likes": {$gt:10},$or:[{"name":"Madrid"}]})
```
...
IMPORTANTE: use ``multi:true`` para actualizar todas las coincidencias. (En caso contrario solo se modifica un documento)
...
Descargue este conjunto de datos en tu HOME (copie este enlace: https://raw.githubusercontent.com/uiuc-cse/data-fa14/gh-pages/data/iris.csv):
...
```
mongoimport -d azureuser -c <your collection> --type csv --file /tmp/iris.csv --headerline
```
